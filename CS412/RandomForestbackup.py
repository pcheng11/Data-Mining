...
def predict(test_data_file, forest):
	...
	print('{0} test data correct!'.format(correct_num))
	print('accuracy is {0}'.format(correctness))
	temp_list = []
...
def accuracy(confusion_dic):
	All = 0
	for i in confusion_dic.keys():
		for j in range(0, len(confusion_dic[i])):
			All += confusion_dic[i][j]

	for label in confusion_dic.keys():
		TP = confusion_dic[label][label-1]
		TN = 0
		for i in confusion_dic.keys():
			if i != label:
				for j in range(0, len(confusion_dic[i])):
					if j != label - 1:
						TN += confusion_dic[i][j]
		FP = 0
		for i in confusion_dic.keys():
			if i != label:
				FP += confusion_dic[i][label-1]

		FN = 0
		for predict_label in range(0, len(confusion_dic[label])): 
			if predict_label+1 != label:
				FN += confusion_dic[label][predict_label]
		N = 0
		for i in confusion_dic.keys():
			if i != label:
				for j in range(0, len(confusion_dic[i])):
		...
		if precision == 'UNDEF' or recall == 'UNDEF':
			f1_score = 'UNDEF'
			f2_score = 'UNDEF'
			f5_score = 'UNDEF'
			...
			f2_score = round(5*precision * recall/(4*precision + recall), 4)
			f5_score = round(1.25*precision * recall/(0.25*precision + recall), 4)
		else:
			f1_score = 'UNDEF'
			f2_score = 'UNDEF'
			f5_score = 'UNDEF'

		print('accuracy for class {0}: {1}'.format(label, accuracy))
		print('specificity for class {0}: {1}'.format(label, specificity))
		print('precision for class {0}: {1}'.format(label, precision))
		print('recall for class {0}: {1}'.format(label, recall))
		print('F-1 score for class {0}: {1}'.format(label, f1_score))
		print('F-2 score for class {0}: {1}'.format(label, f2_score))
		print('F-0.5 score for class {0}: {1}'.format(label, f5_score))
		print('\n')
...
if 'nursery' in training_file:
	...
	print('tree: {0}, sample size: {1}, feature bagging: {2}'.format(240, 1740, 'on'))
	con = predict(testing_file, forest)
	accuracy(con)

if 'led' in training_file:
	
	forest = build_randomForest(training_file, 130, 155, 1)
	print('tree: {0}, sample size: {1}, feature bagging: {2}'.format(130, 155, 'on'))
	con = predict(testing_file, forest)
	accuracy(con)
...
if 'balance.scale' in training_file:
	...
	print('tree: {0}, sample size: {1}, feature bagging: {2}'.format(150, 20, 'on'))
	con = predict(testing_file, forest)
	accuracy(con)

if 'synthetic.social' in training_file:
	forest = build_randomForest(training_file, 300, 2500, 1)
	print('tree: {0}, sample size: {1}, feature bagging: {2}'.format(300, 2000, 'on'))
	con = predict(testing_file, forest)
	accuracy(con)
end = time.time()
